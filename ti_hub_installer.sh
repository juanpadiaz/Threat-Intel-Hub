...
configure_system() {
    ...
    echo
    echo -e "${CYAN}=== CONFIGURACIÓN DE APIS DE THREAT INTELLIGENCE ===${NC}"
    ...
    # AlienVault OTX
    echo
    echo "📌 AlienVault OTX (Open Threat Exchange):"
    echo "   - Proporciona IoCs y pulsos de amenazas"
    echo "   - API gratuita con registro en https://otx.alienvault.com"
    ...
    echo "   - Plataforma de intercambio de inteligencia de amenazas"
    echo "   - Requiere instancia MISP propia o acceso a una compartida"
    read -p "¿Tiene acceso a una instancia MISP? (y/N): " has_misp
    if [[ $has_misp =~ ^[Yy]$ ]]; then
        read -p "MISP URL (ej: https://misp.empresa.com): " MISP_URL
        read -p "MISP API Key: " MISP_API_KEY
        read -p "¿Verificar certificado SSL? (Y/n): " misp_ssl
        MISP_VERIFY_SSL=$([[ $misp_ssl =~ ^[Nn]$ ]] && echo "false" || echo "true")
    fi
    
    # VirusTotal (opcional)
    echo
    echo "📌 VirusTotal (Opcional):"
    echo "   - Enriquecimiento de IoCs"
    echo "   - API gratuita: 4 requests/minuto"
    read -p "¿Tiene API Key de VirusTotal? (y/N): " has_vt
    if [[ $has_vt =~ ^[Yy]$ ]]; then
    ...
    # Wazuh
    echo
    echo -e "${CYAN}=== INTEGRACIÓN CON SIEM ===${NC}"
    ...
    if [[ $setup_wazuh =~ ^[Yy]$ ]]; then
        ...
        echo
        read -p "Wazuh Indexer URL (ej: https://wazuh.local:9200): " WAZUH_INDEXER_URL
        read -p "Wazuh Indexer Usuario [admin]: " WAZUH_INDEXER_USER
        WAZUH_INDEXER_USER=${WAZUH_INDEXER_USER:-admin}
        read -sp "Wazuh Indexer Contraseña: " WAZUH_INDEXER_PASSWORD
        echo
    fi
...
epss_check_hours = 4
misp_priority = ${MISP_URL:+true}
misp_priority = ${MISP_URL:-false}
...
[misp]
enabled = ${MISP_URL:+true}
enabled = ${MISP_URL:-false}
url = $MISP_URL
...
[virustotal]
enabled = ${VT_API_KEY:+true}
enabled = ${VT_API_KEY:-false}
api_key = $VT_API_KEY
...
[email]
enabled = ${SMTP_SERVER:+true}
enabled = ${SMTP_SERVER:-false}
