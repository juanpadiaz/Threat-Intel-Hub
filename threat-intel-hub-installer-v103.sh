...
SQLEOF
    ...
    if [[ "$HAS_WAZUH" == "true" ]]; then
        cat >> /tmp/ti_hub_setup.sql << 'WAZUHEOF'
...
WAZUHEOF
    ...
    cat >> /tmp/ti_hub_setup.sql << 'FINALEOF'
            ...
            if response.status_code == 200:
                ...
                for vuln in vulnerabilities:
                    ...
                    if not existing:
                        ...
                        logger.info(f"📧 Enviando alerta pendiente para KEV {cve_id}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for item in data.get('data', [])[:1000]:  # Limitar a top 1000
                    ...
                    if old_data and old_data['epss_score']:
...
create_admin_commands() {
    ...
    cat > /usr/local/bin/ti-hub-admin << 'ADMINEOF'
#!/bin/bash

# Threat Intel Hub - Herramientas Administrativas v1.0.3

SCRIPT_DIR="/var/lib/threat-intel-hub/scripts"
...
"
        ;;
    "sync-kev")
        echo "🎯 Sincronizando KEV manualmente..."
        sudo -u ti-hub python3 -c "
import sys
sys.path.append('$SCRIPT_DIR')
from ti_hub_monitor import ActionableIntelligence
monitor = ActionableIntelligence()
monitor.check_kev_additions()
print('✅ Sincronización KEV completada')
"
        ;;
    "health-check")
...
ADMINEOF
    ...
    cat > /usr/local/bin/ti-hub-status << 'STATUSEOF'
#!/bin/bash
...
show_summary() {
    ...
    if [[ -n "$SENDER_EMAIL" ]]; then
        ...
        echo env/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
...
Group=ti-hub
WorkingDirectory=/opt/threat-intel-hub
Environment="PATH=/opt/threat-intel-hub/v